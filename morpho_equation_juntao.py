...
class Morpho(PDE):
    ...
    g : float, str
        Gravity
    h : float, Sympy Symbol/Expr, str
    depth of the fluid
    e_s : float, Sympy Symbol/Expr, str
        Diffusivity
    dim : int
    ...
    def __init__(self, nu, rho=1, g = 9.8, dim=2, time=True):
        # set params
        self.dim = dim
        self.time = time
        ...
        self.equations["concentration"] = (
            c.diff(t) 
            + u * c.diff(x)/100000.
            + v * c.diff(y)/100000.
            - ((nu * c).diff(x)).diff(x)/100000./100000.
            - ((nu * c).diff(y)).diff(y)/100000./100000.
        )
